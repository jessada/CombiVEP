...
def generate_figures():
    header_data = np.loadtxt(dev_const.PUB_CONDEL_PREDICTION_RESULT, dtype='S20')[:, :5]
    scores_data = np.loadtxt(dev_const.PUB_CONDEL_PREDICTION_RESULT, dtype='S20')[:, 5:13].astype(np.float)
    ...
    ax.legend(bbox_to_anchor=(0.999, 0.999), loc=1)
    fig.tight_layout()
    fig.savefig(dev_const.PUB_SCORE_DISTRIBUTION_FIG, bbox_inches='tight', pad_inches=0.05)

    return (dev_const.PUB_ROC_FIG,
            dev_const.PUB_AUC_FIG,
            dev_const.PUB_SCORE_DISTRIBUTION_FIG,
            )
...
def generate_precision_performance_report():
    prediction_result  = np.loadtxt(dev_const.PUB_CONDEL_PREDICTION_RESULT, dtype='S20')
