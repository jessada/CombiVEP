...
class TestUpdater(test_template.SafeRefDBTester):
    ...
    def test_ready1(self):
        ...
        self.assertEqual(updater.check_new_file('135'),
                         None,
                         msg='Something went wrong in checking if the updater is ready to work')
...
#        self.individual_debug = True
    ...
    def test_full_update1(self):
        ...
        new_file, new_version = self.__ucsc_updater.check_new_file('135')
        self.assertTrue(new_file.endswith('.sql'),
                        msg='some thing went wrong in UCSC updating process: new file is not the correct file')
        self.assertEqual(new_version,
                         '137',
                         msg='some thing went wrong in UCSC updating process: incorrect new version number')
        downloaded_file = self.__ucsc_updater.download_new_file()
        self.assertTrue(os.path.exists(downloaded_file),
                        msg='some thing went wrong in UCSC updating process: file %s does not exist' % (downloaded_file))
    ...
    def test_parse(self):
        ...
        self.assertEqual(out['135'],
                         'snp135.sql',
                         msg='UCSC parser does not work correctly in finding UCSC files')
...
class TestLJBUpdater(test_template.SafeRefDBTester):
    ...
    def test_full_update1(self):
        ...
        self.assertTrue(new_file.endswith('.readme.txt'),
                        msg='some thing went wrong in LJB updating process: new file is not the correct file')
        self.assertEqual(new_version,
                         '1.3',
                         msg='some thing went wrong in LJB updating process: incorrect new version number')
        downloaded_file = self.__ljb_updater.download_new_file()
        self.assertTrue(os.path.exists(downloaded_file),
                        msg='some thing went wrong in LJB updating process: file %s does not exist' % (downloaded_file))
    ...
    def test_parse(self):
        ...
        self.assertEqual(out['1.3'],
                         ...
                         msg='LJB parser does not work correctly in finding LJB files')
...
class TestMisc(test_template.SafeRefDBTester):
    ...
    def test_unzip(self):
        ...
        self.assertEqual(out[0],
                         os.path.join(self.working_dir, 'try.in'),
                         msg='some files are missing')
        self.assertEqual(out[1],
                         os.path.join(self.working_dir, 'search_dbNSFP_light_v1.3.readme.pdf'),
                         msg='some files are missing')
        self.assertEqual(out[2],
                         os.path.join(self.working_dir, 'search_dbNSFP_light_v1.3.readme.doc'),
                         msg='some files are missing')
        self.assertEqual(out[3],
                         os.path.join(self.working_dir, 'search_dbNSFP_light13.class'),
                         msg='some files are missing')
        self.assertEqual(out[4],
                         os.path.join(self.working_dir, 'dbNSFP_light_v1.3.readme.txt'),
                         msg='some files are missing')
        self.assertEqual(out[5],
                         os.path.join(self.working_dir, 'abc/tryhg19.in'),
                         msg='some files are missing')
        for out_file in out:
