...
class TestUcscReader(SafePreProcTester):
    ...
    def test_formatting(self):
        ...
        for rec in self.__ucsc_reader.fetch_hash_snps('3', 110030157, 110030158):
            readable = True
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_UCSC_CHROM], 'chr3', "Incorrect UCSC formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_UCSC_START_POS], '110030157', "Incorrect UCSC formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_UCSC_END_POS], '110030158', "Incorrect UCSC formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_UCSC_STRAND], '+', "Incorrect UCSC formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_UCSC_REF], 'C', "Incorrect UCSC formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_UCSC_OBSERVED], 'C/T', "Incorrect UCSC formatting")
            break
...
class TestLjbReader(SafePreProcTester):
    ...
    def test_formatting(self):
        ...
        for rec in self.__ljb_reader.fetch_hash_snps('3', 108541778, 108541779):
            readable = True
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_LJB_CHROM], '3', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_LJB_POS], '108541778', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_LJB_REF], 'T', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_LJB_ALT], 'C', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SCORES_SECTION][cbv_const.KEY_PHYLOP_SCORE], '0.102322', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SCORES_SECTION][cbv_const.KEY_SIFT_SCORE], '0.91', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SCORES_SECTION][cbv_const.KEY_PP2_SCORE], '0', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SCORES_SECTION][cbv_const.KEY_LRT_SCORE], '0.312516', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SCORES_SECTION][cbv_const.KEY_MT_SCORE], '0.000000', "Incorrect LJB formatting")
            self.assertEqual(rec[cbv_const.KEY_SCORES_SECTION][cbv_const.KEY_GERP_SCORE], '-3.16', "Incorrect LJB formatting")
            break
...
class TestVcfReader(SafePreProcTester):
    ...
    def test_formatting(self):
        ...
        for rec in self.__vcf_reader.fetch_hash_snps():
            readable = True
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_VCF_CHROM], '1', "Incorrect VCF formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_VCF_POS], '887560', "Incorrect VCF formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_VCF_REF], 'A', "Incorrect VCF formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_VCF_ALT], 'C', "Incorrect VCF formatting")
            break
...
class TestCbvReader(SafePreProcTester):
    ...
    def test_formatting(self):
        ...
        for rec in self.__cbv_reader.fetch_hash_snps():
            readable = True
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_CBV_CHROM], '1', "Incorrect CBV formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_CBV_POS], '19566382', "Incorrect CBV formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_CBV_REF], 'T', "Incorrect CBV formatting")
            self.assertEqual(rec[cbv_const.KEY_SNP_INFO_SECTION][cbv_const.KEY_CBV_ALT], 'C', "Incorrect CBV formatting")
            self.assertEqual(rec[cbv_const.KEY_PREDICTION_SECTION][cbv_const.KEY_CBV_TARGETS], '1', "Incorrect CBV formatting")
            break
