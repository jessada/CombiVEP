...
class TestConfigure(SafeGeneralTester):
    ...
    def test_initial(self):
        ...
        self.__cfg.cfg_file = output_file
        #run test
        self.__cfg.write_ljb_cfg('4.5', '/home/jessada/development/scilifelab/projects/CombiVEP/combivep/refdb/test/data/ljb_controller/dummy_dbNSFP_light1.4')
        self.assertTrue(filecmp.cmp(output_file, expected_out_file),
    ...
    def test_load(self):
        ...
        self.__cfg.cfg_file = test_file
        #run test
        out = self.__cfg.load_cfg()
        self.assertEqual(out[cbv_const.LATEST_UCSC_DB_VERSION],
                         '7.5',
                         "Configure cannot load configuration correctly")
        self.assertEqual(out[cbv_const.LATEST_UCSC_FILE_NAME],
                         'ucsc_file7.5.txt',
                         "Configure cannot load configuration correctly")
        self.assertEqual(out[cbv_const.LATEST_LJB_DB_VERSION],
                         '4.4',
                         "Configure cannot load configuration correctly")
    ...
    def test_update_ucsc_cfg(self):
        ...
        self.copy_file(test_file, output_file)
        #run test
        self.__cfg.write_ucsc_cfg('7.6', 'ucsc_file7.6.txt')
        self.assertTrue(filecmp.cmp(output_file, expected_out_file),
